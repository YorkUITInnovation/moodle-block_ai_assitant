        ...
        foreach ($section_mods as $cmid) {
            ...
            if (in_array($mod[1]->modname, $accepted_modules)) {
                ...
                switch ($mod[1]->modname) {
                    case 'page':
                        $course_structure->sections[$i]->modules[$x]->content = set_module_content(
                            $mod[0]->id,
                            $mod[0]->name,
                            $mod[0]->intro,
                            $mod[0]->content,
                    ...
                    case 'book':
                        ...
                        $content = get_book_content($mod[0]->id);
                        $course_structure->sections[$i]->modules[$x]->content = set_module_content(
                            $mod[0]->id,
                            $mod[0]->name,
                            $mod[0]->intro,
                        ...
                        break;
                    case 'resource': // File
                        $course_structure->sections[$i]->modules[$x]->content = get_files_from_resource(
                            $mod[1]->id,
                            $mod[0]->id
                        );
                        break;
                    case 'folder':
                        ...
                        break;
                    case 'glossary':
                        $content = get_glossary_entries($mod[1]->id, $mod[0]->id, $mod[0]->name);
                        $course_structure->sections[$i]->modules[$x]->content = set_module_content(
                            $mod[0]->id,
                            $mod[0]->name,
                            $mod[0]->intro,
...
function set_module_content($id, $name, $intro, $content, $module_type)
{
    $module = new stdClass();
    // Set the file name
 ...
 * @return stdClass
 */
function get_glossary_entries($cmid, $id, $name)
{
    global $CFG, $DB;
 ...
 * @return array
 */
function get_files_from_resource($cmid, $id)
{
    global $CFG;
 ...
 * @return stdClass
 */
function get_folder_files($cmid, $id, $name, $intro)
{
    global $CFG, $DB;
